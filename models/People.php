    ...
    public function updatePeopleList()
    {
        ...
        while ($i < $people_count) {
            ...
            $fullName           = str_replace("'", "\'",
                $people_array[0]['fullName']);
            $link               = $people_array[0]['link'];
            $firstName          = str_replace("'", "\'",
                $people_array[0]['firstName']);
            $lastName           = str_replace("'", "\'",
                $people_array[0]['lastName']);
            $primaryNumber      = $people_array[0]['primaryNumber'] ?? 0;
            $birthDate          = $people_array[0]['birthDate'];
            $currentAge         = $people_array[0]['currentAge'];
            $birthCity          = str_replace("'", "\'",
                $people_array[0]['birthCity']);
            $birthStateProvince = str_replace("'", "\'",
                $people_array[0]['birthStateProvince'] ?? NULL);
            $birthCountry       = str_replace("'", "\'",
                $people_array[0]['birthCountry']);
            $nationality        = $people_array[0]['nationality'] ?? NULL;
            $height             = str_replace(array("'", '"'), array("\'", '\"'),
                $people_array[0]['height']);
